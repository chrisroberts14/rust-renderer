...
impl GpuRasterRenderer {
    ...
    async fn init_async() -> Self {
        ...
        let (device, queue) = adapter.request_device(&wgpu::DeviceDescriptor::default()).await.expect("Failed to get device");
    ...
    fn create_gpu_framebuffer(device: &wgpu::Device, w: u32, h: u32) -> GpuFramebuffer {
        ...
        GpuFramebuffer { colour, colour_view, depth, depth_view, readback, width: w, height: h }
    }
    ...
    fn upload_object(
        ...
        for (face_idx, &(i0, i1, i2)) in obj.mesh.faces.iter().enumerate() {
            let (uv_i0, uv_i1, uv_i2) = obj.mesh.uv_faces
                .get(face_idx)
                .copied()
                .unwrap_or((0, 0, 0));
            ...
            Material::Texture(tex) => (tex.width, tex.height, {
                ...
                for y in 0..tex.height {
                    for x in 0..tex.width {
                        data.extend_from_slice(&tex.sample(
    ...
    fn render_objects(
...

        {
            ...
            for obj in objects {
                ...
                let (_tex, tex_view, sampler) = Self::get_or_create_texture(&self.device, &self.queue, &obj.material);
                let bind_group   = Self::build_bind_group(
                    &self.device, &self.pipeline,
                    &uniform_buf, &light_buf, &tex_view, &sampler,
                );
        ...
        self.device.poll(wgpu::PollType::Wait { submission_index: Some(submission_index), timeout: None }).expect("GPU poll failed");
...

        {
            ...
            for y in 0..h as usize {
                ...
                for x in 0..w as usize {
                    let base = x * 4;
                    framebuffer.set_pixel(x, y, [src_row[base], src_row[base+1], src_row[base+2], src_row[base+3]]);
                }
            }
        }
        gpu_fb.readback.unmap();

        RenderStats { triangle_count: objects.iter().map(|o| o.mesh.faces.len()).sum(), tile_count: 0 }
    }

    fn render_wireframe(&self, objects: &[Object], camera: &Camera, framebuffer: &Framebuffer) -> RenderStats {
        todo!()
    }
}
